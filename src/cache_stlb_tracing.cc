		...
		for (uint32_t i=0; i<MAX_READ; i++) {
			...
			if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
				...
				if (way >= 0) { // read hit
					...
					// check mshr
					uint8_t miss_handled = 1;
					int mshr_index = check_mshr(&RQ.entry[index]); //miss status holding register
					...
					if(mshr_index == -2)
					{
						...
						else
						{
							...
							if (lower_level) // Prob == not llc/stlb
								lower_level->add_rq(&RQ.entry[index]);
							else {
								if (cache_type == IS_STLB) {
									...
									if(unsigned(RQ.entry[index].instruction) != 0){
										...
										stlb_misses[0]++; //Data misses sto stlb
									}
									...
									if(iflag == 0){ //	Was iflag==1 -> now prefetching for data
										free_indexes = sorted_free_distances();
