...
if [ "$#" -ne 9 ]; then
    ...
    echo "Usage: ./build_champsim.sh [branch_pred] [l1d_pref] [l2c_pref] [llc_pref] [llc_repl] [num_core] [stlb_pref] [build_name]"
    exit 1
fi
...
BINARY_NAME="${STLB_PREFETCHER}-${BUILD_NAME}-${BRANCH}-${L1I_PREFETCHER}-${L1D_PREFETCHER}-${L2C_PREFETCHER}-${LLC_PREFETCHER}-${LLC_REPLACEMENT}-${NUM_CORE}core"
