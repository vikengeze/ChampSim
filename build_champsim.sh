...
if [ "$#" -ne 8 ]; then
    ...
    echo "Usage: ./build_champsim.sh [branch_pred] [l1d_pref] [l2c_pref] [llc_pref] [llc_repl] [num_core] [stlb_pref]"
    exit 1
fi
...
BINARY_NAME="${STLB_PREFETCHER}-${BRANCH}-${L1I_PREFETCHER}-${L1D_PREFETCHER}-${L2C_PREFETCHER}-${LLC_PREFETCHER}-${LLC_REPLACEMENT}-${NUM_CORE}core"

echo "Binary: bin/${BINARY_NAME}"
echo ""
mv bin/champsim bin/${BINARY_NAME}

mkdir -p statistics/${STLB_PREFETCHER}
